...
const char * const BITCOIN_CONF_FILENAME = "aif.conf";
const char * const BITCOIN_PID_FILENAME = "aifd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "aif" is a composite category enabling all Aif-related debug output
                if(ptrCategory->count(std::string("aif"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AifCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AifCore
    // Mac: ~/Library/Application Support/AifCore
    // Unix: ~/.aifcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AifCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AifCore";
#else
    // Unix
    return pathRet / ".aifcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty aif.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override aif.conf
