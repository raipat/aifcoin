...
*Setup instructions for a Gitian build of Aif Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Aif
...
- [Building Aif Core](#building-aif-core)
...
Clone the git repositories for Aif Core and Gitian.
...
in the Aif Core repository under 'Fetch and create inputs' to install sources which require
...
To build Aif Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Aif Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/aif/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/raipat/aifcoin
...
For example:
```bash
URL=https://github.com/crowning-/aif.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit aif=${COMMIT} --url aif=${URL} ../aif/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit aif=${COMMIT} --url aif=${URL} ../aif/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit aif=${COMMIT} --url aif=${URL} ../aif/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the aif git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../aif/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/aif
SIGPATH=/some/root/path/aif-detached-sigs

./bin/gbuild --url aif=${BTCPATH},signature=${SIGPATH} ../aif/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/aif-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/aif-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/aif-osx-build.assert
```
...
[aif/gitian.sigs](https://github.com/aifdev/gitian.sigs/) repository, or if that's not possible to create a pull
request.
